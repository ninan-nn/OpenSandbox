...
- In Node.js, every `Sandbox` and `SandboxManager` clones the base `ConnectionConfig` via `withFreshTransport()`, so each instance gets an isolated `undici` keep-alive pool. Call `sandbox.close()` or `manager.close()` when you are done so the SDK can release the associated agent.
