...
export class ConnectionConfig {
  ...
  private _sseFetch: typeof fetch;
  ...
  constructor(opts: ConnectionConfigOptions = {}) {
    ...
    this._fetch = fetch;
    this._sseFetch = fetch;
    this._closeTransport = async () => {/* No-op: Browser fetch has no pooled dispatcher to close. */};
  ...
  /**
   * Clone this configuration while allocating a fresh transport.
   *
   * Each clone builds its own HTTP/SSE helpers so callers can close them without
   * affecting other SDK clients.
   */
