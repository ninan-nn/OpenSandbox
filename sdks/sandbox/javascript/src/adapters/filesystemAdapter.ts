...
function toReadableStream(it: AsyncIterable<Uint8Array>): ReadableStream<Uint8Array> {
...
function toPermission(e: { mode?: number; owner?: string; group?: string }): Permission {
  return {
    mode: e.mode ?? 755,
    owner: e.owner,
    group: e.group,
...
export class FilesystemAdapter implements SandboxFiles {
  ...
  private static readonly Api = {
    ...
    SearchFilesOk: null as unknown as ExecdPaths["/files/search"]["get"]["responses"][200]["content"]["application/json"],
    FilesInfoOk: null as unknown as ExecdPaths["/files/info"]["get"]["responses"][200]["content"]["application/json"],
    MakeDirsRequest: null as unknown as ExecdPaths["/directories"]["post"]["requestBody"]["content"]["application/json"],
    SetPermissionsRequest: null as unknown as ExecdPaths["/files/permissions"]["post"]["requestBody"]["content"]["application/json"],
    MoveFilesRequest: null as unknown as ExecdPaths["/files/mv"]["post"]["requestBody"]["content"]["application/json"],
    ReplaceContentsRequest: null as unknown as ExecdPaths["/files/replace"]["post"]["requestBody"]["content"]["application/json"],
  };
  ...
  private static readonly _ApiFileInfo = null as unknown as (
    typeof FilesystemAdapter.Api.SearchFilesOk
  )[number];

  private mapApiFileInfo(
    raw: typeof FilesystemAdapter._ApiFileInfo,
  ): FileInfo {
    const {
      path,
      size,
  ...
  async createDirectories(entries: Array<Pick<WriteEntry, "path" | "mode" | "owner" | "group">>): Promise<void> {
  ...
  async setPermissions(entries: SetPermissionEntry[]): Promise<void> {
    ...
    const body = req as unknown as typeof FilesystemAdapter.Api.SetPermissionsRequest;
  ...
  async moveFiles(entries: MoveEntry[]): Promise<void> {
    ...
    const body = req as unknown as typeof FilesystemAdapter.Api.MoveFilesRequest;
  ...
  async replaceContents(entries: ContentReplaceEntry[]): Promise<void> {
    ...
    const body = req as unknown as typeof FilesystemAdapter.Api.ReplaceContentsRequest;
  ...
  async readBytes(path: string, opts?: { range?: string }): Promise<Uint8Array> {
  ...
  private async *downloadStream(path: string, opts?: { range?: string }): AsyncIterable<Uint8Array> {
    const url = joinUrl(this.opts.baseUrl, "/files/download") + `?path=${encodeURIComponent(path)}`;
    const res = await this.fetch(url, {
      method: "GET",
      headers: {
  ...
  async readFile(path: string, opts?: { encoding?: string; range?: string }): Promise<string> {
