...
- **Lifecycle**: `CodeInterpreter` wraps an existing `Sandbox` instance and reuses its connection configuration. Each sandbox instance clones the transport via `ConnectionConfig.withFreshTransport()`, so call `sandbox.close()` when you are finished to release the Node.js keep-alive agent and avoid leak.
