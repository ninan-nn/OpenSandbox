...
- **生命周期**：`CodeInterpreter` 基于既有的 `Sandbox` 实例进行包装，并复用其连接配置。SDK 会通过 `ConnectionConfig.withFreshTransport()` 为每个实例复刻 Transport，完成交互后请调用 `sandbox.close()` 释放 Node.js 的 keep-alive agent，以避免资源泄漏。
